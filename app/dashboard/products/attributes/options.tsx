...
export default function OptionsComponent({ attribute }: VariantAttribute) {
    const [attributes, setAttributes] = useState<VariantAttribute[]>([])
    const [selectedAttributes, setSelectedAttributes] = useState<number[]>([])
    const [isAttributeFormOpen, setIsAttributeFormOpen] = useState(false)
    const [isEditing, setIsEditing] = useState(false)
    const [attributeToAction, setAttributeToAction] = useState<VariantAttribute | null>(null)
                ...
                <Tabs value={viewMode} onValueChange={setViewMode} className="w-full">
                    <div className="flex justify-between md:flex-row flex-col mb-6">
                        ...
                        <TabsList className="grid w-auto grid-cols-2">
                            <TabsTrigger value="attribute" className="text-xl">
                                {/* <List className="h-4 w-4 mr-2" /> */}
                                Attributes
                            </TabsTrigger>
                            <TabsTrigger value="option" className="text-xl">
